...
#        help="data input file(s). This is an alternative to simply specifying files if more than one data input file is required (not implemented yet)")
    ...
    parser.add_argument('-L','--layout', type=str, nargs='+', default='generic', ##
        help="layout for the plots. E.g. 'document': serif,LaTeX,pdf ; 'slides': sans serif, big, png ; 'generic': slides + pdf. This is not implemented yet.")
    parser.add_argument('-P','--plots', type=str, nargs='+', default = plots,
        help="do only this plot/these plots")
    parser.add_argument('-n', '--normalize', action='store_true',
        help="normalize Monte Carlo samples to the event count in data and regardless of the given luminosity. This is only applied to data/MC comparisons")
...
def labels(ax, opt=options(), jet=False, bin=None, result=None, legloc='best', frame=False):
    ...
    if jet: jetlabel(ax, opt.algorithm, opt.correction)    #on demand
    binlabel(ax, bin)
...
def jetlabel(ax, algorithm="", correction="", posx = 0.05, posy = 0.95):
    ...
    elif correction.find("L1L2L3")>=0:
        corr = r"L1L2L3 corrected"
    elif correction.find("L1L2")>=0:
        corr = r"L1L2 corrected"
    elif correction.find("L1")>=0:
        corr = r"L1 corrected"
    else:
        corr = r"uncorrected"
    if algorithm.find("ak5PFJets")>=0:
...
def genericplot(quantity, q, obj, fdata, custom_keys_data, fmc, custom_keys_mc, factor, stg, legloc='center right'):
    #print q, "of the", obj    
    print "This function 'genericplot' is deprecated and has moved to 'plotdatamc.datamcplot'"
    ...
    histo00 = ta.bar(histo_mc.x, histo_mc.y,(histo_mc.x[2]-histo_mc.x[1]),bottom =numpy.ones(len(histo_mc.x))*1e-6, fill=True, facecolor=stg.mcColor, edgecolor=stg.mcColor)
...
def AddAlgoAndCorrectionCaption(ax, algo = "ak5PFJets"):
    print "This function 'AddAlgoAndCorrectionCaption' is deprecated and now called 'jetlabel'"
    jetlabel(ax, algo, algo)

def AxisLabels(ax, q='resp', obj='jet'):
    print "The function 'AxisLabels' is now deprecated, please use 'axislabel' instead."
    axislabel(ax,q,obj)

def GetNameFromSelection(quantity='zmass', common={}, variation={}):
    print "This function 'GetNameFromSelection' is deprecated and now called 'getROOT.GetHistoname'"
