...
def extrapolateRatio(fdata, fmc,
    ...
    for x in var:
        # get the histograms
        folder_var = folder + str(x).replace(".", "_")   # 0.3 -> 0_3
        hist_respdata = getroot.getobject(fdata, folder_var + "/" + histo)
        hist_respmc = getroot.getobject(fmc, folder_var + "/" + histo)
    ...
    hist_zpt = getroot.getobject(fdata, folder_var +
                                 "/z_pt_ak5PFJetsL1L2L3CHS_hist")
...
def extrapolate_ratio(method, fdata, fmc, opt, tag="extrapol",
                      folder_prefix=""):
    bins = opt.bins
    if bins[0] == 0:
        bins.pop(0)
    ...
    for s in str_bins:
        ...
        yex, yex_err, x = extrapolateRatio(fdata, fmc,
            ...
            method + "_" + opt.algorithm + opt.correction + "_hist")
        ratioEx.y += [yex]
    ...
    # TODO (Joram): use plotbase.labels
    ...
    ax.minorticks_on()
    plotbase.Save(fig, method + "_ratio_" + opt.algorithm +
                  opt.correction + "_ratio_beforextrapol", opt, False)


def response(rootfile, method='jetresp', extrapol=None,
             algorithm=plotbase.options().algorithm,
             correction=plotbase.options().correction, fit=None):
    # extrapolation = None keine, 'ratio' neu,
    # F'response' extrapolate response and then ratio
...
def response_ratio(fdata, fmc, method='jetresp', extrapol=None,
        algorithm=plotbase.options().algorithm,
        correction=plotbase.options().correction, fit=None):
