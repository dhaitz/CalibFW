...
fdata = getROOT.OpenFile(plotBase.GetPath() + "closure_data_2011A.root", (settings.verbosity > 1))
fmc = getROOT.OpenFile(plotBase.GetPath() + "closure_mc_DY2011.root", (settings.verbosity > 1))
...
def balex(level="L1L2L3", res=""):
    tf, ta, tname = plotBase.makeplot("jetresp_mc")
    rootgraph = extrapolate_ratio("balresp", "AK5PFJetsCHS"+level, tf, ta, True, "extrapol", res)
    #AddEtaRange(ta, "$|\eta| < 1.3$")
    namo = "jetresp_ratio_ak5PFJets"+level+res+"_data_mc_ratio_extrapol"
    plotBase.Save(tf, name, settings, False)
    rootgraph.Print( name + "_root.png" )
    rootgraph.Print( name + "_root.root" )

# MPF 

def mpfex(level="L1L2L3", res=""):
    tf, ta, tname = plotBase.makeplot("jetresp_mc")
    rootgraph = extrapolate_ratio("mpfresp", "AK5PFJetsCHS"+level, tf, ta, False, "no_extrapol", res)
