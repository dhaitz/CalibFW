...
def zmass_qualitycuts(fdata, fmc, opt):
    datamcplot('z_mass', 'mass', 'Z', fdata, fmc, opt, 'center right',
               {'incut': 'qualitycuts'}, log=True)


# Leading jet
def jetpt(fdata, fmc, opt):
    datamcplot('jet1_pt', 'pt', 'jet', fdata, fmc, opt, 'center right',
               log=True)
...
if __name__ == "__main__":
    ...
    balance(fdata, fmc, opt=plotbase.options(bins=bins))
    mpf(fdata, fmc, opt=plotbase.options(bins=bins))
