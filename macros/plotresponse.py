...
def extrapol(files, opt, 
    ...
    for cut, ax1, ax2, subtext1, subtext2 in zip(variations, axes[:l], axes[l:], subtexts[:l], subtexts[l:]):
        ...
        pt_eta_label = r"$p_\mathrm{T}^\mathrm{Z}>30 \ \mathrm{GeV}$   $\eta^\mathrm{Jet1}<1.3$"
        ax1.text(0.03, 0.97, pt_eta_label, va='top', ha='left', color='black', transform=ax1.transAxes, size='large')
        ...
        plotbase.plotdatamc.datamcplot('ptbalance_alpha', files, local_opt, legloc='lower left',
                   ...
                   subtext="", fig_axes=(fig, ax2),fit='intercept', ratio=True, fit_offset=-0.1)
        ...
        if extrapolate_mpf ==True:
            plotbase.plotdatamc.datamcplot(mpftype+'_alpha', files, local_opt, legloc='lower left',
                           changes=changes, rebin=rebin, file_name = "", subplot=True, xy_names=['alpha','datamcratio'],
                           subtext=subtext2, fig_axes=(fig, ax2),fit='intercept', ratio=True, fit_offset=-0.2)
        else:
        ...
        ax2.text(0.03, 0.97, pt_eta_label, va='top', ha='left', color='black', transform=ax2.transAxes, size='large')
