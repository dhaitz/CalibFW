...
public:
	...
	Hist2D(std::string sName, std::string sFolder,
			double xlow, double xtop, double ylow, double ytop):
			HistBase<Hist2D>(sName, sFolder),
...
public:
		...
		m_bUseCustomBin(false)
	{
		...
		if (m_bUseCustomBin)
		{
			m_hist.reset(RootFileHelper::GetStandaloneTH1D_1(
				GetName(), GetCaption(), m_iBinCount, &m_dCustomBins[0]));
		...
		{
			m_hist.reset(RootFileHelper::GetStandaloneTH1D_2(
...
{
public:
	DrawHist1dConsumerBase() : m_hist(NULL) {}
...
//class ModHistLabels : public ModifierBase<Hist1D>
//{
//public:
//	ModHistLabels(std::string xlabel, std::string ylabel="Events"):
//			m_xlabel(xlabel), m_ylabel(ylabel)
//	{}

//	virtual void AfterDataEntry(Hist1D* pElem)
//	{
//		pElem->m_xlabel = this->m_xlabel;
//		pElem->m_ylabel = this->m_ylabel;
//	}
...
{
public:
	ModHistBinRange(double lower, double upper):
