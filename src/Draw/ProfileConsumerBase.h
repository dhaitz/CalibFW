...
public:
	...
	ProfileConsumerBase(SourceTypeForThis* xsource, SourceTypeForThis* ysource,
			std::string plotName) : m_plotName(plotName)
	{
	...
	virtual void Init(PipelineTypeForThis * pset)
	{
		...
		if (m_xsource->HasDefaultBins())
			modlist.push_back(new ModProfileXBinRange(
					m_xsource->GetDefaultLowBin(), m_xsource->GetDefaultHighBin()
			));

		m_profile.reset(new Profile2d(m_plotName,
			...
			FilterResult& result)
	{
		...
		double xval;
		double yval;
		bool has_xval = m_xsource->GetValue(
				event, metaData, this->GetPipelineSettings(), xval);
		bool has_yval = m_ysource->GetValue(
				event, metaData, this->GetPipelineSettings(), yval);

		if (has_xval && has_yval)
			m_profile->AddPoint(xval, yval, metaData.GetWeight());
