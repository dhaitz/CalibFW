...
CModBinRange::CModBinRange( double lower, double upper)
{
    ...
    this->m_dBinUpper = upper;
}

void CModBinRange::ModifyBeforeHistCreation( void * pB  )
{
    CHistDrawBase * pDrawBase = (CHistDrawBase *) pB ;

    pDrawBase->m_dBinLower = this->m_dBinLower;
...
CModBinCount::CModBinCount( int binCount)
{
    this->m_iBinCount = binCount;
}

void CModBinCount::ModifyBeforeHistCreation( void * pB  )
{
    CHistDrawBase * pDrawBase = (CHistDrawBase *) pB ;
...
void CModHorizontalLine::ModifyAfterHistoDraw( TCanvas * pC, TH1 * pHist )
{
  TLine * line = new TLine( this->m_xVal, 
			    0.0, 
			    this->m_xVal, 
			    pHist->GetMaximum() );
