// Inspired by jsonxx from Hong Jiang <hong@hjiang.net>
...
bool Json::parse(std::istream& input, Json& object) {
...
bool Value::parse(std::istream& input, Value& value) {
    ...
    value.object_value_ = new Json();
    if (Json::parse(input, *value.object_value_)) {
        value.type_ = OBJECT_;
        return true;
    }
...
std::ostream& operator<<(std::ostream& stream, const Value& v) {
    ...
    } else if (v.is<Value::Null>()) {
        return stream << "null";
    } else if (v.is<Json>()) {
        return stream << v.get<Json>();
    } else if (v.is<Array>()){
...
std::ostream& operator<<(std::ostream& stream, const Array& v) {
    stream << "[";
    const std::vector<Value*>& values(v.values());
...
std::ostream& operator<<(std::ostream& stream, const Json& v) {
    stream << "{";
    const std::map<std::string, Value*>& kv(v.kv_map());
    for (std::map<std::string, Value*>::const_iterator i = kv.begin();
         i != kv.end(); /**/) {
