...
class ZJetCutBase : public LocalProducerBase<ZJetEventData, ZJetProduct, ZJetPipelineSettings>
{
public:
...
public:
	...
	virtual void PopulateLocal(ZJetEventData const& data, ZJetProduct const& product,
							   ZJetProduct::LocalproductType& localproduct,
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		KDataLVs muons;
		if (m_pipelineSettings.GetGenCuts())
			muons = product.GetAsLVs(product.m_genMuons);
		else
			muons = product.GetAsLVs(product.m_listValidMuons);
		...
		localproduct.SetCutResult(this->GetId(), allPassed);
	}
...
public:
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		...
		for (KDataLVs::const_iterator it = muons.begin(); it != muons.end(); it++)
		{
			...
			{
				localproduct.SetCutResult(this->GetId(), false);
				return;
			}
		}

		localproduct.SetCutResult(this->GetId(), true);
	}
...
public:
	...
	virtual void PopulateLocal(ZJetEventData const& data, ZJetProduct const& product,
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		if (!product.HasValidKindZ(m_pipelineSettings.GetGenCuts()))
			return;
		localproduct.SetCutResult(this->GetId(),
								  product.GetRefKindZ(m_pipelineSettings.GetGenCuts()).p4.Pt()
...
public:
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		if (!product.HasValidKindZ(m_pipelineSettings.GetGenCuts()))
			return;
		localproduct.SetCutResult(this->GetId(),
								  std::abs(product.GetRefKindZ(m_pipelineSettings.GetGenCuts()).p4.mass() - g_kZmass)
...
public:
	...
	virtual void PopulateLocal(ZJetEventData const& data, ZJetProduct const& product,
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		if (!product.HasValidJet(m_pipelineSettings, data))
			return;

		KDataLV* jet = product.GetValidPrimaryJet(m_pipelineSettings, data);
		localproduct.SetCutResult(this->GetId(),
...
public:
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		if (!product.HasValidKindZ(m_pipelineSettings.GetGenCuts()))
			return;
		if (product.GetValidJetCount(m_pipelineSettings, data) < 2)
		{
			localproduct.SetCutResult(this->GetId(), true);
			return;
		}

		KDataLV* jet2 = product.GetValidJet(m_pipelineSettings, data, 1);

		localproduct.SetCutResult(this->GetId(),
								  (product.GetBalance(jet2, product.GetPtKindZ(m_pipelineSettings.GetGenCuts()))
...
public:
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		if (!product.HasValidJet(m_pipelineSettings, data))
			return;
		KDataLV* jet1 = product.GetValidPrimaryJet(m_pipelineSettings, data);

		if (!product.HasValidKindZ(m_pipelineSettings.GetGenCuts()))
			return;
		double zphi = product.GetRefKindZ(m_pipelineSettings.GetGenCuts()).p4.Phi();
...
public:
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		...
		if (!product.HasValidKindZ(m_pipelineSettings.GetGenCuts()))
			return;
		double jeta = product.GetValidPrimaryJet(m_pipelineSettings, data)->p4.Eta();
		double zeta = product.GetRefKindZ(m_pipelineSettings.GetGenCuts()).p4.Eta();
...
public:
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		...
		KDataLV* jet = product.GetValidJet(m_pipelineSettings, data, 1);
		localproduct.SetCutResult(this->GetId(),
...
public:
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		...
		if (product.GetValidJetCount(m_pipelineSettings, data) < 3)
		{
		...
		KDataLV* jet1 = product.GetValidPrimaryJet(m_pipelineSettings, data);
		KDataLV* jet2 = product.GetValidJet(m_pipelineSettings, data, 1);
		KDataLV* jet3 = product.GetValidJet(m_pipelineSettings, data, 2);
		...
		localproduct.SetCutResult(this->GetId(),
								  std::abs(product.GetZeppenfeld(jet1, jet2, jet3)) < 1.0
								 );
	}
...
public:
	...
	{
		bool allPassed = (
							 (std::abs(product.leadingeminus.p4.Eta()) < m_pipelineSettings.GetCutElectronEta())
							 && (std::abs(product.leadingeplus.p4.Eta()) < m_pipelineSettings.GetCutElectronEta())
						 );
...
public:
	...
	{
		bool allPassed = (
							 (product.leadingeminus.p4.Pt() > m_pipelineSettings.GetCutElectronPt())
							 && (product.leadingeplus.p4.Pt() > m_pipelineSettings.GetCutElectronPt())
						 );
...
public:
							   ...
							   ZJetPipelineSettings const& m_pipelineSettings) const
	{
		...
		for (auto e : product.m_listValidElectrons)
			allPassed = allPassed
