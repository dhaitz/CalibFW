...
// JET Correction
	...
	std::cout << "HIST: " << histData.size();
	    ...
	    //p_dataCalibPoints->SetPoint(i, 1.0f + i* 30.0f, i * 0.1f + 1.0f );
...
*/
	h_corr.addLatex(0.08,0.01, "Fit function: " + plot_function, true);
        ...
        if ( g_correction_level == 3 )
            saveHolder(h_corr,img_formats, false, "_l3", sGlobalPrefix + funcName);
        if ( g_correction_level == 2 )
...
TString g_l3_corr_formula;
vdouble g_l3_corr_params;
...
//
    ...
    // level3 
    g_l3_corr =  TF1( "l3_corr", g_l3_corr_formula );
    
    int i = 0;
    BOOST_FOREACH( double d, g_l3_corr_params)
    {
      g_l3_corr.SetParameter(i, d);
      i++;
    }
...
//             getResponses(responses,responses_points_all_bins,ifile,*interval,algo);
            ...
            {
		//double val = respo->GetMean();
		/*
		if ( g_correction_level == 3 )
		{
		  val = g_l3_corr.Eval(val) * val;
		} 
		  */
...
//             h_resp.normalizeHistos();
	    ...
	    if ( g_correction_level == 3 )	    
	      saveHolder(*c_dataBinned,img_formats, false, "_l3", sGlobalPrefix);
...
*/
        ...
        if ( g_correction_level == 3 )
            saveHolder(h_response,img_formats, false, "_l3", sGlobalPrefix);
        if ( g_correction_level == 2 )
                  ...
                  TString algoname)
{
    ...
    for (int ievt=0;ievt< tree->GetEntries();++ievt) {
        ...
        if (interval.contains(Z->Pt())) {
            ...
            if ( g_correction_level == 3 )
            {
                // use l3 corrected jet !!
                response = ( jet->Pt() * l2corr ) /Z->Pt();
		
		std::cout << "L3 correcting response: " << response << "  factor: " <<  g_l3_corr.Eval(response) << std::endl;
		
		response = response * g_l3_corr.Eval(response);
            }	  
