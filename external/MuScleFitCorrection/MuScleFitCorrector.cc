...
scaleFunctionBase<double*>* MuScleFitCorrector::getScaleFunction()
{
	return scaleFunction_;
}
...
double MuScleFitCorrector::getCorrectPt(const RMDataLV& lorentzVector, const int& chg) const
{
...
double MuScleFitCorrector::getSmearedPt(const RMDataLV& lorentzVector, const int& chg, bool fake) const
{
	double pt = lorentzVector.Pt();
...
// Method to apply correction to a RMDataLV&
void MuScleFitCorrector::applyPtCorrection(RMDataLV& lorentzVector, const int& chg) const
{
	double corrPt = getCorrectPt(lorentzVector, chg);
	lorentzVector.SetPt(corrPt);
}

// Method to apply smearing to a RMDataLV&
void MuScleFitCorrector::applyPtSmearing(RMDataLV& lorentzVector, const int& chg, bool fake) const
{
	double smearedPt = getSmearedPt(lorentzVector, chg, fake);
	lorentzVector.SetPt(smearedPt);
}

std::vector<double> MuScleFitCorrector::getResolMCParVec()
{
	return resolMCParVec_;
}
...
void MuScleFitCorrector::convertToArrays()
{
	...
	{
		std::cout << "Error: inconsistent number of parameters: scaleFunction has "
				  << scaleParNum << " parameters but " << scaleParVecSize
				  << " have been read from file" << std::endl;
		exit(1);
	}
